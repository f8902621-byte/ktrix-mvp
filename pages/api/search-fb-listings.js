...
export default async function handler(req, res) {
  ...
  const { city, district, propertyType, priceMin, priceMax } = req.body;
  ...
  try {
    ...
    if (priceMin && Number(priceMin) > 0) {
      // price en millions, priceMin en tỷ
    ...
    // Mapper vers le format attendu par search.js
    const results = (data || []).map(listing => ({
      id: listing.id,
      source: 'facebook',
      title: listing.title || 'Annonce Facebook',
      price: listing.price ? listing.price * 1000000 : null, // millions → VND
      area: listing.area,
      ...
      imageUrl: null, // pas de photo pour les listings FB
      url: listing.url,
